<script lang="ts">
  ...
  async function handleAddSchedule() {
    ...
    setTimeout(() => { newScheduleId = null; }, 1600);
  }
</script>
  ...
  {:else}
    {#each sorted as schedule (schedule.id)}
      <div id="schedule-{schedule.id}">
        <ScheduleCard {schedule} {onplay} highlight={schedule.id === newScheduleId} />
      </div>
    {/each}
  {/if}

  <button class="add-btn" onclick={handleAddSchedule}>
    <IconCalendarPlus size={18} /> {tr.schedule.addSchedule}
  </button>
</div>
...
<style>
  .list {
    ...
    padding: 20px; max-width: 960px; margin: 0 auto; width: 100%;
  }
