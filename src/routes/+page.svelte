<script lang="ts">
  ...
  import { IconPlayerPause, IconPlayerPlay, IconSettings } from '@tabler/icons-svelte';
  ...
  onMount(() => {
    ...
    await listen<{ scheduleId: string; minutesBefore: number }>('scheduler:notify', ({ payload }) => {
      const schedule = configStore.schedules.find(s => s.id === payload.scheduleId);
      if (!schedule) return;
    ...
    await listen('playback:pause', () => setPlaybackState({ status: 'paused' }));
    await listen('playback:resume', () => setPlaybackState({ status: 'playing' }));
  ...
  let playQueue: { path: string; volume: number; loopCount: number; type: 'video' | 'audio' }[] = [];
  let queueIndex = 0;
  let loopRemaining = 0;
  ...
  async function startScheduledPlayback(scheduleId: string) {
    ...
    setPlaybackState({ status: 'playing', scheduleId, mediaIndex: 0, currentLoop: 0 });
  ...
  async function advancePlaybackQueue() {
    loopRemaining--;
    if (loopRemaining > 0) { await playQueueItem(queueIndex); return; }
    queueIndex++;
...
<svelte:head><title>Playback Announcer</title></svelte:head>
...
<div class="app-shell">
  <header class="header">
    <div class="header-left">
      ...
      >{schedulerStatus === 'active' ? tr.status.active : tr.status.paused}</span>
    </div>
    <div class="header-actions">
      {#if isDirty}
        <button class="btn btn-ghost" onclick={revertConfig}>{tr.actions.revert}</button>
        <button class="btn btn-primary" onclick={saveConfig}>{tr.actions.save}</button>
      {/if}
      <button
        class="btn btn-ghost btn-icon"
        onclick={handleToggleScheduler}
        title={schedulerStatus === 'active' ? 'Jeda semua jadwal' : 'Aktifkan semua jadwal'}
      >{#if schedulerStatus === 'active'}<IconPlayerPause size={16} />{:else}<IconPlayerPlay size={16} />{/if}</button>
      <button class="btn btn-ghost btn-icon" onclick={openSettings} title={tr.settings.title}><IconSettings size={16} /></button>
    </div>
  </header>

  <main class="main-scroll"><ScheduleList onplay={startScheduledPlayback} /></main>

  <footer class="footer">&copy; 2026 Andi &ndash; Disperpusip Jawa Timur. All rights reserved.</footer>
...
<style>
  .app-shell { display: flex; flex-direction: column; height: 100vh; overflow: hidden; background: var(--color-bg); }
  .header {
    display: flex; align-items: center; justify-content: space-between;
    padding: 12px 20px; background: var(--color-surface);
    border-bottom: 1px solid var(--color-border); flex-shrink: 0; gap: 12px;
  }
  ...
  .footer {
    padding: 8px 20px; text-align: center; font-size: 11px;
